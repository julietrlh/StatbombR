---
title: "Eventlog - focus on specific action - Final"
...
```{r}
#WC2023_Spain_lastmatch_dataframe <- readRDS("~/5A/Research_project/Eventlog/Dataframes/WC2023_Spain_lastmatch_dataframe.RData")
# 
...
```{r}
WC2023_Spain_lastmatch_dataframe <- readRDS("/Users/lalylacroix/Desktop/INSA/5A/Projet/WC2023/Eventlog + Dataframes/WC2023_Spain_lastmatch_dataframe.RData")
...
# ex <- WC2023_Spain_lastmatch_dataframe[, c("timestamp","type.name", "player.name","player.id","period","team.name","shot.type.name","shot.type.id","shot.technique.name","shot.outcome.name","goalkeeper.type.name","goalkeeper.outcome.name")]
...
```{r}
ev_log2 <- WC2023_Spain_lastmatch_dataframe[, c("timestamp","type.name", "player.name","player.id","period","team.name","shot.type.name","shot.type.id","shot.technique.name","shot.outcome.name")]
...
**Exemple 1**  
Séquences des types d'actions les plus intéressantes (environ les 2 à 4 qui précèdent) : (EVENTLOG 3 notamment)
> avant un tir sauvé par le gardien 
> avant un shot 
...
```{r}
ev_log3 <- WC2023_Spain_lastmatch_dataframe[, c("timestamp","type.name", "player.name","team.name","play_pattern.name","shot.outcome.name","out","pass.outcome.name","pass.type.name","position.name","location.x")]
...
# Sélectionner les indices des lignes précédentes (10 lignes avant chaque "Shot" avec les outcomes spécifiés)
selected_rows <- lapply(saved_shot_indices, function(i) {
  start_index <- max(1, i - 4)  # negation que l'indice de départ n'est pas négatif
  end_index <- i
  return(start_index:end_index)
})
...
**CAS 4**
case_id_relevant : 2 actions avant une sortie de ballon hors des limites du terrain 
...
# Sélectionner les indices des lignes précédentes (10 lignes avant chaque occurrence "Out")
...
```{r}
WC2023_Spain_firstmatch_dataframe <- WC2023_Spain_lastmatch_dataframe %>%
  mutate(possession_team.name = str_remove(possession_team.name, " Women's"))

ev_log4 <- WC2023_Spain_lastmatch_dataframe[, c("timestamp","type.name","possession_team.name", "player.name","play_pattern.name")]
...
```{r}
ev_log5_complete <- WC2023_Spain_lastmatch_dataframe[,c("timestamp","player.name","type.name","possession_team.id", "possession_team.name")]
...
```{r}
ev_log11 <- WC2023_Spain_lastmatch_dataframe[, c("timestamp","type.name", "player.name","position.name","period","team.name")]
...
```{r}
# Trouver les indices des lignes 
left_back_indices <- which(ev_log11$position.name == "Left Center Back" )
# | ev_log3$pass.type.name == "Goal Kick" )
...
for (i in 2:nb_li ) { 
  if ( !is.na(ev_log11_from_left_back$position.name[i]) && ev_log11_from_left_back$position.name[i] == "Left Center Back" ) { 
    ev_log11_from_left_back$case_id_relevant[i] <- ev_log11_from_left_back$case_id_relevant[i-1] +1
  }
  else if (!is.na(ev_log11_from_left_back$position.name[i]) && ev_log11_from_left_back$position.name[i] != "Left Center Back" ) {
...
```{r}
ev_log11_goalkeeper <- WC2023_Spain_lastmatch_dataframe[, c("timestamp","type.name", "player.name","position.name","period","team.name","location.x")]
...
```{r}
ev_log3_goal %>% activity_presence() %>%
    plot
```
...
ATTENTION : uniquement lorsque l'on a un statut start + complete pour les différentes instances d'activités afin de pouvoir évaluer la durée de chacune.
...
Peut être calculé à différents niveaux : log, trace, case
